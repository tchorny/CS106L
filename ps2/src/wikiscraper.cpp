...
bool noInvalidChar(char c) {return ((c == ':') || (c == '#')) ? false : true;}

unordered_set<string> findWikiLinks(const string& inp) {

    unordered_set<string> wikiLinks;
    const string toSearch = "<a href=\"/wiki/";
    auto start = inp.begin();
    auto end = inp.end();

    while(start != end) {

        auto linkStart = search(start, end, toSearch.begin(), toSearch.end());
        if(linkStart == end) break;
        linkStart += toSearch.end() - toSearch.begin();
        auto linkEnd = find(linkStart, end, '\"');
        if(linkEnd == end) break;

        if(all_of(linkStart, linkEnd, noInvalidChar)) {
            string link;
            copy(linkStart, linkEnd, back_inserter(link));
